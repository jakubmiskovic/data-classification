...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="Popis",
        help="Name of the text feature in the dataset (defaults to Slovak 'Popis').",
    )
    parser.add_argument(
        "--label_column",
        type=str,
        default="filter",
        help="Name of the label feature in the dataset (defaults to Slovak 'filter').",
    )
    parser.add_argument(
        "--filter_list_file",
        type=str,
...
def drop_missing_labels(raw_datasets: DatasetDict, label_column: str, splits_with_labels: set):
    for split in list(splits_with_labels):
        ...
        filtered = dataset.filter(has_label)
        removed = len(dataset) - len(filtered)
        if removed:
            print(f"Removed {removed} rows without '{label_column}' from split '{split}'")

        raw_datasets[split] = filtered
...
def prepare_data(args: argparse.Namespace) -> Tuple:
    ...
    drop_missing_labels(raw_datasets, args.label_column, splits_with_labels)
